...
class ExecutionRouter:
    ...
    async def execute(self, action: ActionRequest) -> ExecutionResult:
        """
        Actually run the action against its executor, bypassing the
        BLOCK/NEED_APPROVAL gating in route(). Used directly by the
        approval-decision flow once a NEED_APPROVAL action has been
        approved -- at that point the original decision is stale (it was
        NEED_APPROVAL) and re-running it through route() would just
        produce another "pending approval" stub instead of executing.
        """
