...
`ask_user_low_confidence` submits an action with confidence below
`settings.ASK_USER_CONFIDENCE_THRESHOLD` (default `0.5`), so the guardrail
routes it to `Decision.ASK_USER` instead of executing it. The question lands
in the `pending_user_questions` queue — inspect it with `GET /api/v1/ask-user`
and resolve it with `POST /api/v1/ask-user/{action_id}/respond`. See
