...
async def _post_openai(
    ...
    async with httpx.AsyncClient(
        ...
        if (
            ...
            logger.warning(
                "LLM router rejected tools parameter (400); retrying without tools"
            )
...
async def _post_gemini(
    ...
    if tool_messages:
        ...
        states = [
            ...
            if state
            and latest_state
            and state["interaction_id"] == latest_state["interaction_id"]
        ]

        interaction_ids = {
            state["interaction_id"] for _, state in states
        }
        ...
        if len(interaction_ids) == 1 and states:
            ...
            body["input"] = [
                {
                    ...
                    "result": _gemini_text_function_result(
                        message.get("content")
                    ),
                }
            ...
            logger.warning(
                "Could not resolve Gemini interaction state for tool result(s)"
            )
